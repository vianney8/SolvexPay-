  ...
  const transferMutation = useMutation({
    ...
    onError: (error: any) => {
      try {
        ...
        toast({ title: "Erreur", description: parsed.message || "Impossible d'initier le transfert.", variant: "destructive" });
      } catch {
      ...
      <div className="max-w-md mx-auto space-y-5">
        {isBlocked && (
          <div className="rounded-2xl bg-red-500/10 border border-red-500/30 p-5 text-center space-y-2">
            ...
            <p className="text-sm text-muted-foreground">Impossible d'effectuer un transfert. Compte suspendu.</p>
          </div>
        )}
        <div
        ...
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription className="text-center">
              Pour effectuer des transferts, votre compte doit être vérifié. Accédez au menu <strong>Vérification KYC</strong> pour soumettre votre dossier.
            </DialogDescription>
          </DialogHeader>
          <div className="flex flex-col gap-2 mt-2">
            <Link href="/kyc" onClick={() => setKycGateOpen(false)}>
              <Button className="w-full font-bold gap-2 bg-orange-500 hover:bg-orange-600 text-white" data-testid="button-kyc-dialog-transfer">
