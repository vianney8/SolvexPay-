...
import { setupAuth, isAuthenticated, isPartner, isAdmin, registerAuthRoutes } from "./replit_integrations/auth";
...
export async function registerRoutes(
  ...
  app.post("/api/admin/partners", isAdmin, async (req, res) => {
    try {
      ...
      const { companyName, email, password, country } = validation.data;
      ...
      res.status(201).json({ id: user.id, email, companyName });
    } catch (error) {
  ...
  app.patch("/api/admin/partners/:id", isAdmin, async (req, res) => {
    try {
      ...
      if (typeof isBlocked !== "boolean") {
        return res.status(400).json({ message: "isBlocked doit être un booléen" });
      }

      await storage.updateUserStatus(id, isBlocked);
  ...
  app.get("/api/partner/profile", isPartner, async (req: any, res) => {
    try {
      const profile = await storage.getPartnerProfile(req.user.id);
  ...
  app.patch("/api/partner/profile", isPartner, async (req: any, res) => {
    try {
      ...
      await storage.updatePartnerProfile(req.user.id, validation.data);
      res.json({ success: true });
    } catch (error) {
  ...
  app.patch("/api/partner/password", isPartner, async (req: any, res) => {
    try {
      ...
      await authStorage.resetPassword(req.user.id, passwordHash);
  ...
  app.get("/api/partner/wallet", isPartner, async (req: any, res) => {
    try {
      const wallet = await storage.getWallet(req.user.id);
      res.json(wallet);
    } catch (error) {
      res.status(500).json({ message: "Erreur wallet" });
    }
  });

  app.get("/api/partner/stats", isPartner, async (req: any, res) => {
    try {
  ...
  app.get("/api/partner/transactions", isPartner, async (req: any, res) => {
    try {
      const transactions = await storage.getTransactions(req.user.id);
      res.json(transactions);
    } catch (error) {
      res.status(500).json({ message: "Erreur transactions" });
    }
  });

  app.get("/api/partner/api-keys", isPartner, async (req: any, res) => {
    try {
      const keys = await storage.getApiKeys(req.user.id);
      res.json(keys);
    } catch (error) {
      res.status(500).json({ message: "Erreur API keys" });
    }
  });

  app.post("/api/partner/api-keys", isPartner, async (req: any, res) => {
    try {
      ...
      res.status(500).json({ message: "Erreur création API key" });
    }
  });

  app.delete("/api/partner/api-keys/:id", isPartner, async (req: any, res) => {
    try {
      const keys = await storage.getApiKeys(req.user.id);
      if (!keys.some(k => k.id === req.params.id)) {
        return res.status(404).json({ message: "Clé introuvable" });
      }
      await storage.deleteApiKey(req.params.id);
      res.json({ success: true });
    } catch (error) {
      res.status(500).json({ message: "Erreur suppression API key" });
    }
  });

  // Direct Pay API v1
  app.post("/api/partner/v1/pay", async (req, res) => {
    try {
      const authHeader = req.headers.authorization;
      if (!authHeader?.startsWith("Bearer ")) {
        return res.status(401).json({ message: "Clé API manquante ou invalide" });
      }
      const fullKey = authHeader.substring(7);
      const apiKeyRecord = await storage.findApiKeyByFullKey(fullKey);

      if (!apiKeyRecord || !apiKeyRecord.isActive || apiKeyRecord.adminLocked) {
        return res.status(401).json({ message: "Clé API invalide ou désactivée" });
      }

      const user = await (await import("./replit_integrations/auth")).authStorage.getUser(apiKeyRecord.userId);
      if (!user || user.isBlocked || !user.isPartner) {
        return res.status(403).json({ message: "Compte partenaire suspendu ou invalide" });
      }
      ...
      const validation = directPaySchema.safeParse(req.body);
      if (!validation.success) {
        return res.status(400).json({ message: validation.error.errors[0].message });
      }
      ...
      const nameParts = (customer_name || "Client Partner").trim().split(" ");
      ...
      const transaction = await storage.createTransaction({
        ...
        reference,
        status: "pending",
        description: description || "Paiement API Partenaire",
        fees: String(fees),
      ...
      res.json({
        ...
        amount,
        fees,
        payment_url: omnipayResponse.payment_url,
      });
    } catch (error: any) {
      console.error("Direct pay error:", error);
      res.status(500).json({ message: error.message || "Erreur lors du paiement" });
    }
  });
